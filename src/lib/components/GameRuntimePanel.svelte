...
<section class="@container card space-y-4 p-4">
  ...
  {#if isHeroic}
    <!-- Heroic reads structured per-game JSON, not a launch string — so the
         useful action is to write the tuning straight into its config. The
         button itself is the shared LauncherAction, which also puts it in the
         command bar; this block keeps the explanation, not a second copy. -->
    <div class="space-y-2 rounded-xl border border-border/60 bg-surface-2/40 p-3">
